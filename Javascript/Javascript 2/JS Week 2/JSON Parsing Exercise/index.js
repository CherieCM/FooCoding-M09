...
const spriteImages = obj.sprites;
function renderImages() {
  const createUl = document.createElement('ul');
  for (let i = 0; i < spriteImages.length; i++) {
    let imgList = document.createElement('li');
    let imgs = document.createElement('img');
    imgList.appendChild(imgs);
    imgList.appendChild(spriteImages);
    createUl.appendChild(imgList);
  }
  document.getElementById('images').appendChild(createUl);
}
renderImages();
