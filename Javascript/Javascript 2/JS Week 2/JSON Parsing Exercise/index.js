...
const pokemon = JSON.parse(fetchPokemonData());

const pokeSprites = Object.values(pokemon.sprites);
// console.log(pokeSprites);
const img = pokeSprites.filter((image) => {
  return image !== null;
});
// console.log(img);

let pokeImages = img.map((pokeSprites) => {
  pokeImg = document.createElement('img');
  pokeImg.src = pokeSprites;
  pokeImg.width = 275;

  document.body.appendChild(pokeImg).style.border = 'thin dotted blue';
});
